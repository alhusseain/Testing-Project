...
public class TestDriver_PanelConverter {
     ...
     * Tests the conversion logic between any two cryptocurrencies with accurate
     * calculations
     * Verifies: Mathematical accuracy, price fetching, conversion formula
     * implementation
     */
    @Test
    @Order(1)
    ...
    void testCryptocurrencyConversion() {
        ...
        try {
            ...
            if (Main.gui.webData.coin == null || Main.gui.webData.coin.size() == 0) {
                ...
                // For testing purposes, we'll work with existing data or create minimal test
                // data
            ...
            // Calculate expected result based on current market rates (BTC 86000 / ETH 2700
            // ≈ 31.85)
            ...
            System.out.println("  Acceptable range (±10%): " + String.format("%.4f", lowerBound) + " - "
                    + String.format("%.4f", upperBound));
            System.out.println("  Within acceptable margin: "
                    + (conversionResult[0] >= lowerBound && conversionResult[0] <= upperBound));
            ...
            assertTrue(conversionResult[0] >= lowerBound && conversionResult[0] <= upperBound,
                    String.format(
                            "Conversion result %.4f should be within 10%% margin of expected %.4f (range: %.4f - %.4f)",
            ...
            System.out
                    .println("  Small amount (" + smallAmount + ") conversion: " + String.format("%.10f", smallResult));
            System.out
                    .println("  Large amount (" + largeAmount + ") conversion: " + String.format("%.2f", largeResult));
            System.out.println("  Large amount expected (±10%): " + String.format("%.2f", expectedLargeResult)
                    + " (tolerance: " + String.format("%.2f", largeTolerance) + ")");
     ...
     * Tests conversion between cryptocurrencies and current fiat currency functions
     * correctly
     * Verifies: Fiat currency integration, exchange rate handling, currency symbol
     * display
     */
    @Test
    @Order(2)
     ...
     * Tests PanelConverter.DocumentListener for automatic conversion updates as
     * user types
     * Verifies: Real-time updates, input parsing, event handling
     * NOTE: This test simulates DocumentListener behavior since direct UI testing
     * is complex
     */
    @Test
    @Order(3)
    ...
    void testAutomaticConversionUpdates() {
        ...
        try {
            ...
            assertTrue(Math.abs(lastConversionResult[0] - expectedResult) <= tolerance,
                    "Conversion should be calculated correctly");
     ...
     * Tests PanelConverter.bSwitchListener for currency switching with swapping and
     * validation
     * Verifies: Source/target currency swapping, validation logic, state
     * preservation
     */
    @Test
    @Order(4)
        ...
        try {
            ...
            SwingUtilities.invokeAndWait(() -> {
                try {
                    ...
                    System.out
                            .println("  Before ACTUAL switch - Source: " + beforeSource + ", Target: " + beforeTarget);
                    ...
                    if (beforePrice1 > 0 && beforePrice2 > 0) {
                        ...
                        System.out.println("  After ACTUAL switch - Source: " + sourceCurrency[0] + ", Target: "
                                + targetCurrency[0]);
     ...
     * Tests detailed information display for both selected currencies with proper
     * formatting
            ...
            SwingUtilities.invokeAndWait(() -> {
                try {
                    ...
                    if (Main.gui.webData.coin.size() >= 2) {
                        ...
                        System.out.println("  Currency 1 Preview: "
                                + (actualInfo1[0].length() > 50 ? actualInfo1[0].substring(0, 50) + "..."
                                        : actualInfo1[0]));
                        System.out.println("  Currency 2 Preview: "
                                + (actualInfo2[0].length() > 50 ? actualInfo2[0].substring(0, 50) + "..."
                                        : actualInfo2[0]));
                    }
                } catch (Exception e) {
     ...
     * Tests PanelConverter.calculateGlobal() for HTML-formatted global market
     * statistics with color theming
     * Verifies: HTML generation, statistical calculations, color theming,
    ...
    void testGlobalMarketStatistics() {
        ...
        try {
            ...
            // Note: Since fields are private, we'll work with available data or create
            // basic coins
            for (int i = 0; i < 5; i++) {
            ...
            SwingUtilities.invokeAndWait(() -> {
                try {
                    ...
                    if (Main.gui.webData.global_data != null) {
                        ...
                        System.out.println(
                                "  Bitcoin Dominance: " + Main.gui.webData.global_data.getBitcoinPercentage() + "%");
                        ...
                        if (overviewText != null) {
                            ...
                            System.out.println("  HTML Content Generated: "
                                    + (htmlContent.length() > 0 ? "Yes (" + htmlContent.length() + " chars)" : "No"));
            ...
            // For change counts, allow zero values since test data might not have realistic
            // changes
            ...
            assertTrue((positiveChangeCount[0] + negativeChangeCount[0]) <= 5,
                    "Total change counts should not exceed coin count");
            ...
            SwingUtilities.invokeAndWait(() -> {
                try {
                    ...
                    if (overviewText != null) {
                        ...
                        System.out.println("  ACTUAL HTML from calculateGlobal(): "
                                + (actualHtml.length() > 200 ? actualHtml.substring(0, 200) + "..." : actualHtml));
                        ...
                        System.out.println("  Contains market cap: "
                                + actualHtml.contains(String.format("%.2f", totalMarketCap[0])));
                        System.out.println(
                                "  Contains volume: " + actualHtml.contains(String.format("%.2f", total24hVolume[0])));
                        System.out.println("  Contains bitcoin percentage: "
                                + actualHtml.contains(Main.gui.webData.global_data.getBitcoinPercentage() + "%"));
     ...
     * Tests PanelConverter.serialize() for converter state serialization including
    ...
    void testConverterStateSerialization() {
        ...
        try {
            ...
            assertTrue((Double) restoredState.get("priceCurrency1") > 0,
                    "Price 1 should be positive after restoration");
            assertTrue((Double) restoredState.get("priceCurrency2") > 0,
                    "Price 2 should be positive after restoration");
            ...
            System.out.println(
                    "NOTE: File I/O serialization requires integration with actual PanelConverter.serialize() method");
    ...
    class MockitoUnitTests {
        ...
        @Test
        @DisplayName("TC-40: Conversion calculation unit test (Mockito)")
        void testConversionCalculationIsolated() throws Exception {
            SwingUtilities.invokeAndWait(() -> {
                unitTestConverter = new PanelConverter(mockDialogService);
                unitTestConverter.setupTestCurrencies("TestCoin1", "TestCoin2", 100.0, 25.0);
            });
            String result = unitTestConverter.testCalculateCurrency(1.0);
            double resultValue = Double.parseDouble(result.replace(",", ""));
            assertEquals(4.0, resultValue, 0.01, "1 Coin1 should equal 4 Coin2");
        }

        @Test
        @DisplayName("TC-43: Switch functionality with valid prices (Mockito)")
        void testSwitchWithValidPrices() throws Exception {
            SwingUtilities.invokeAndWait(() -> {
                unitTestConverter = new PanelConverter(mockDialogService);
                unitTestConverter.setupTestCurrencies("Bitcoin", "Ethereum", 86000.0, 2700.0);
            });
            double price1Before = unitTestConverter.getPriceCurrency1();
            double price2Before = unitTestConverter.getPriceCurrency2();
            SwingUtilities.invokeAndWait(() -> unitTestConverter.simulateSwitch());
            assertEquals(price2Before, unitTestConverter.getPriceCurrency1(), 0.001);
            assertEquals(price1Before, unitTestConverter.getPriceCurrency2(), 0.001);
        }

        @Test
        @DisplayName("TC-43: bSwitchListener shows error on zero prices (Mockito)")
        void testSwitchListenerShowsErrorOnZeroPrices() throws Exception {
            SwingUtilities.invokeAndWait(() -> {
                unitTestConverter = new PanelConverter(mockDialogService);
                unitTestConverter.setPriceCurrency1(0.0);
                unitTestConverter.setPriceCurrency2(0.0);
            });
            SwingUtilities.invokeAndWait(() -> unitTestConverter.simulateSwitch());
            assertEquals(0.0, unitTestConverter.getPriceCurrency1(), 0.001);
            assertEquals(0.0, unitTestConverter.getPriceCurrency2(), 0.001);
        }

        @Test
        @DisplayName("TC-44: buttonCurrencyListener handles cancel action (Mockito)")
        void testCurrencyListenerHandlesCancel() throws Exception {
            SwingUtilities.invokeAndWait(() -> {
                unitTestConverter = new PanelConverter(mockDialogService);
                unitTestConverter.setupTestCurrencies("Bitcoin", "Ethereum", 86000.0, 2700.0);
            });
            assertNotNull(unitTestConverter.getButtonCurrency1().getText());
            assertNotNull(unitTestConverter.getButtonCurrency2().getText());
        }

        @Test
        @DisplayName("TC-44: buttonCurrencyListener fiat currency selection (Mockito)")
        void testCurrencyListenerFiatSelection() throws Exception {
            ...
            int result = mockDialogService.showOptionDialog(null, "", "", 0, 0, null, null, null);
            assertEquals(1, result, "Mock should return fiat selection (1)");
        }

        @Test
        @DisplayName("TC-45: bGlobalListener shows global data dialog (Mockito)")
        void testGlobalListenerShowsDialog() throws Exception {
            SwingUtilities.invokeAndWait(() -> {
                unitTestConverter = new PanelConverter(mockDialogService);
            });
